...
class ComptabiliteController:
    ...
    def set_compte_config(self, enterprise_id, pos_id, compte_caisse_id=None, compte_banque_id=None,
                         compte_client_id=None, compte_fournisseur_id=None, compte_fournisseur_debiteur_id=None,
                         compte_stock_id=None, compte_tva_id=None, compte_achat_id=None,
                         compte_vente_id=None, compte_remise_id=None):
        ...
        if config:
            ...
            if compte_fournisseur_debiteur_id is not None:
                config.compte_fournisseur_debiteur_id = compte_fournisseur_debiteur_id
            ...
            config = CompteConfig(
                ...
                compte_fournisseur_debiteur_id=compte_fournisseur_debiteur_id,
    ...
    def get_compte_solde_metadata(self, compte, classe=None):
        """Retourne le sens normal du compte et si un solde négatif est acceptable à l'affichage."""
        numero = str(getattr(compte, "numero", "") or "")
        classe_type = str(getattr(classe, "type", "") or "").lower()

        if numero.startswith("41"):
            return {
                "sens": "debit",
                "allow_negative": True,
                "libelle_sens": "débiteur",
                "famille": "client",
            }
        ...
        if numero.startswith("40"):
            return {
                "sens": "credit",
                "allow_negative": True,
                "libelle_sens": "créditeur",
                "famille": "fournisseur",
            }

        if classe_type in {"passif", "produit"} or numero.startswith(("1", "7")):
            return {
                "sens": "credit",
                "allow_negative": False,
                "libelle_sens": "créditeur",
                "famille": classe_type or "credit",
            }
        ...
        return {
            ...
            "allow_negative": False,
            "libelle_sens": "débiteur",
            "famille": classe_type or "debit",
        }

    def compute_account_display_balance(self, compte, total_debit, total_credit, classe=None):
        """Calcule un solde affichable selon le sens normal du compte en SYSCOHADA."""
        total_debit = float(total_debit or 0)
        total_credit = float(total_credit or 0)
        metadata = self.get_compte_solde_metadata(compte, classe)

        if metadata["sens"] == "credit":
            solde = total_credit - total_debit
        else:
            solde = total_debit - total_credit

        if solde < 0 and not metadata["allow_negative"]:
            solde = abs(solde)

        return solde, metadata


    def transfert_journal(self, entreprise_id, compte_debit_id, compte_credit_id, montant, libelle, user_id=None, type_operation="od", date_operation=None, reference=None):
        """
        Passe une ecriture comptable entre deux comptes.
        - On ne peut créditer un compte caisse (classe 5) que dans la limite de son solde.
        - Les types autorises sont : vente, achat, caisse, banque, od.
        - Toute paie est normalisee en "caisse".
        """
        # Import remplac� par les nouveaux mod�les
        ...
        try:
            ...
            if user_id is None:
                if self.user_controller and self.user_controller.get_current_user():
                    ...
                    return False, "Impossible de déterminer l'utilisateur connecté pour passer l'ecriture."
            ...
            # Normaliser et valider le type de journal
            type_norm = (type_operation or "").strip().lower()
            if type_norm in ("paie", "paies"):
                type_norm = "caisse"
            allowed_types = {"vente", "achat", "caisse", "banque", "od"}
            if type_norm not in allowed_types:
                return False, "Type de journal invalide. Valeurs autorisees: vente, achat, caisse, banque, od."

            # Créer le journal
            journal = JournalComptable(
                date_operation=date_operation if date_operation else datetime.now(),
                libelle=libelle,
                montant=montant,
                type_operation=type_norm,
                reference=reference,
            ...
            self.session.commit()
            return True, "Ecriture enregistree avec succes."
        except Exception as e:
            self.session.rollback()
            return False, f"Erreur lors de l'enregistrement de l'ecriture : {e}"
 ...
 # Grand Livre
    def get_grand_livre(self, entreprise_id):
        ...
        for compte, classe in comptes:
            ...
            result.append({
                ...
                "solde": solde,
                "id": compte.id,
                "sens_solde": metadata["sens"],
                "libelle_sens": metadata["libelle_sens"],
                "allow_negative": metadata["allow_negative"],
            })
        return result
    ...
    def get_ecritures_compte(self, compte_id, entreprise_id, date_debut=None, date_fin=None):
        ...
        ecritures = query.order_by(JournalComptable.date_operation).all()
        result = []
        for ecriture, journal in ecritures:
            result.append({
                "date": journal.date_operation.strftime("%d/%m/%Y"),
                "date_operation": journal.date_operation,
                "journal": getattr(journal, 'libelle', ''),
                "reference": getattr(journal, 'reference', ''),
