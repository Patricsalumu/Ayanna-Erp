    ...
    def create_expense(self, expense_data):
        ...
        try:
            with self.db_manager.get_session() as session:
                ...
                # Si un compte financier est fourni dans expense_data, on enregistre
                # une écriture dans le journal. Le type est déterminé automatiquement :
                # compte 57x (SYSCOHADA) → 'banque', sinon → 'caisse'.
                ...
                if compte_financier_id:
                    try:
                        ...
                        compte_financier = session.query(CompteComptable).filter(
                            CompteComptable.id == compte_financier_id
                        ).first()
                        type_journal = self._resolve_journal_type(
                            getattr(compte_financier, 'numero', None)
                        )
                        journal = JournalComptable(
                            enterprise_id=self.pos_id,
                            libelle=expense_data.get('description', ''),
                            montant=float(expense_data.get('amount', 0.0)),
                            type_operation=type_journal,
                            reference=f"EXP-{expense.id}",
                            description=f"Dépense boutique ID: {expense.id} - {expense_data.get('expense_type', '')}",
                            user_id=expense_data.get('user_id', 1),
                            date_operation=expense_data.get('expense_date', datetime.now())
                        )
                        session.add(journal)
                        session.commit()
                        print(f"✅ Écriture journal ({type_journal}): EXP-{expense.id}")
                    except Exception as e_journal:
                        print(f"⚠️ Écriture journal non enregistrée: {e_journal}")
