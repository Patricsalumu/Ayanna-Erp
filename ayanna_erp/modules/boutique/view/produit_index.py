...
class ProduitIndex(QWidget):
    ...
    def _is_admin(self):
        ...
        if not self.current_user:
            return False
        # Supporter à la fois un dict et un objet ORM User
    ...
    def setup_ui(self):
        ...
        if not self._is_admin():
            self.new_product_btn.setVisible(False)
        ...
        filter_layout.addWidget(QLabel("État stock:"))
        self.stock_status_combo = QComboBox()
        self.stock_status_combo.addItem("Tous", None)
        self.stock_status_combo.addItem("Stock normal", "normal")
        self.stock_status_combo.addItem("Stock faible", "faible")
        self.stock_status_combo.addItem("En rupture", "rupture")
        self.stock_status_combo.currentTextChanged.connect(self.refresh_products)
        ...
        self.products_table.setHorizontalHeaderLabels([
            "ID", "Nom", "Catégorie", "Prix", "Stock", "Stock min", "Unité", "Compte produit", "Compte charge", "Statut", "Actions"
        ])
        ...
        # === PARTIE DROITE : DÉTAILS PRODUIT (visible uniquement pour admin/super_admin) ===
        ...
        # Connexion du clic sur le tableau (détails visibles uniquement pour admin)
        if self._is_admin():
            self.products_table.cellClicked.connect(self.on_product_selected)
        # Connexion du double-clic pour édition (admin uniquement)
    ...
    def on_product_selected(self, row, column):
        ...
        with self.db_manager.get_session() as session:
            ...
            details += f"<b>Stock minimum (POS) :</b> {stock_info['min_stock']}<br>"
    ...
    def _sync_stock_min_to_warehouse(self, session, product_id, min_stock_value):
        """Synchroniser le stock minimum du produit avec la table stock_produit_entrepot"""
        try:
            from ayanna_erp.modules.stock.models import StockProduitEntrepot, StockWarehouse
            ...
            if not pos_warehouse:
                print(f"⚠️  Entrepôt {warehouse_code} introuvable pour synchroniser le stock minimum")
                return
            
            # Mettre à jour le stock pour ce produit dans l'entrepôt POS
            stock_entry = session.query(StockProduitEntrepot).filter_by(
                product_id=product_id, 
                warehouse_id=pos_warehouse.id
            ).first()
            ...
            if stock_entry:
                stock_entry.min_stock_level = min_stock_value
                session.commit()
                print(f"✅ Stock minimum ({min_stock_value}) synchronisé pour le produit {product_id} dans l'entrepôt {warehouse_code}")
            else:
                print(f"⚠️  Pas d'entrée stock trouvée pour le produit {product_id} dans l'entrepôt {warehouse_code}")
        except Exception as e:
            print(f"❌ Erreur lors de la synchronisation du stock minimum: {e}")
    ...
    def refresh_products(self):
        ...
        try:
            with self.db_manager.get_session() as session:
                ...
                stock_filter = self.stock_status_combo.currentData()
                if stock_filter:
                    filtered_products = []
                    for product in products:
                        info = self._get_product_stock_info(product.id)
                        stock_total = info.get('total_stock', 0) or 0
                        stock_min = info.get('min_stock', 0) or 0

                        if stock_filter == "rupture" and stock_total == 0:
                            filtered_products.append(product)
                        elif stock_filter == "faible" and (stock_total > 0 and stock_total < stock_min):
                            filtered_products.append(product)
                        elif stock_filter == "normal" and (stock_total > 0 and stock_total >= stock_min):
                            filtered_products.append(product)
    ...
    def populate_products_table(self, products: List):
        ...
        for row, product in enumerate(products):
            ...
            # Stock
            # Stock + Stock minimum (par entrepôt POS)
            try:
                stock_info = self._get_product_stock_info(product.id)
                stock_total = stock_info.get('total_stock', 0) or 0
                stock_min = stock_info.get('min_stock', 0) or 0

                stock_item = QTableWidgetItem(str(stock_total))
                stock_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)

                # Logique basée sur le stock minimum par entrepôt
                if stock_total == 0:
                    # En rupture de stock
                    stock_item.setBackground(QColor("#E74C3C"))
                    stock_item.setForeground(QColor("white"))
                    stock_item.setToolTip("⚠️ Stock en rupture")
                elif stock_total < stock_min:
                    # Stock faible (inférieur au minimum)
                    stock_item.setBackground(QColor("#F39C12"))
                    stock_item.setForeground(QColor("white"))
                    stock_item.setToolTip(f"⚠️ Stock faible (min: {stock_min})")
                else:
                    # Stock normal (supérieur ou égal au minimum)
                    stock_item.setBackground(QColor("#27AE60"))
                    stock_item.setForeground(QColor("white"))
                    stock_item.setToolTip(f"✅ Stock normal (min: {stock_min})")

                self.products_table.setItem(row, 4, stock_item)

                # Colonne Stock min
                min_item = QTableWidgetItem(str(stock_min))
                min_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                self.products_table.setItem(row, 5, min_item)
            except Exception:
                stock_item = QTableWidgetItem("Err")
                stock_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                self.products_table.setItem(row, 4, stock_item)
                min_item = QTableWidgetItem("Err")
                min_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                self.products_table.setItem(row, 5, min_item)

            # Unité
            self.products_table.setItem(row, 6, QTableWidgetItem(getattr(product, "unit", "unité")))
            ...
            self.products_table.setItem(row, 7, compte_produit_item)
            ...
            self.products_table.setItem(row, 8, compte_charge_item)
    ...
    def create_new_product(self):
        if not self._is_admin():
            QMessageBox.warning(self, "Accès refusé", "Seuls les administrateurs peuvent créer des produits.")
            return
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            try:
                with self.db_manager.get_session() as session:
                    ...
                    self._sync_stock_min_to_warehouse(session, new_product.id, product_data.get("stock_min", 0))
    ...
    def edit_product(self, product):
        if not self._is_admin():
            QMessageBox.warning(self, "Accès refusé", "Seuls les administrateurs peuvent modifier des produits.")
            return
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            try:
                with self.db_manager.get_session() as session:
                    ...
                    if updated_product:
                        # Mettre à jour le min_stock_level dans stock_produit_entrepot
                        self._sync_stock_min_to_warehouse(session, updated_product.id, product_data.get("stock_min", 0))
    ...
    def toggle_product_status(self, product):
        if not self._is_admin():
            QMessageBox.warning(self, "Accès refusé", "Seuls les administrateurs peuvent activer/désactiver des produits.")
            return
...
class ProductFormDialog(QDialog):
    ...
    def __init__(self, parent=None, produit_controller=None, product=None):
        ...
        self.setModal(True)
        self.setMinimumSize(550, 550)  # Taille plus grande pour les onglets
        self.resize(550, 600)  # Taille par défaut
        self.setup_ui()
        if self.is_editing:
