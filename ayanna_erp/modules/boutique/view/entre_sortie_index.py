"""
Journal de Caisse - Module Boutique
...
class EntreeSortieIndex(QWidget):
    ...
    def load_journal_data(self):
        ...
        - shop_expenses (sorties)
        - shop_payments (entrées) - si disponibles
        ...
        try:
            ...
            # Charger les sorties (dépenses) depuis shop_expenses
            try:
                pos_id = getattr(self.main_controller, 'pos_id', 1)
                expenses = self.expense_controller.get_expenses_by_date_and_pos(
                    selected_date, 
                    pos_id
                )
                ...
                for expense in expenses:
                    entry = {
                        ...
                        'categorie': getattr(expense, 'expense_type', 'Dépense'),
                        'montant_entree': 0.0,
                        'montant_sortie': float(expense.amount),
                        'utilisateur': getattr(expense, 'user_name', 'Utilisateur'),
            ...
            # TODO: Charger les entrées (paiements) depuis shop_payments quand cette table sera créée
    ...
    def export_to_pdf(self):
        ...
        try:
            ...
            qdate = self.date_filter.date()
            selected_date = date(qdate.year(), qdate.month(), qdate.day())
            filename = f"journal_caisse_boutique_{selected_date.strftime('%Y%m%d')}.pdf"
            ...
            # Titre
            title = Paragraph(f"Journal de Caisse Boutique - {selected_date.strftime('%d/%m/%Y')}", title_style)
            story.append(title)
