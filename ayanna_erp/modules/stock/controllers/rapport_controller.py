...
class RapportController:
    ...
    def get_recent_reports(self, session=None, limit: int = 20) -> List[Dict[str, Any]]:
        """
        Récupérer les rapports récents
        
        Args:
            session: Session SQLAlchemy (ignorée pour compatibilité)
            limit: Nombre maximum de rapports à récupérer
            
        Returns:
            Liste des rapports récents (pour l'instant simulée)
        """
        try:
            # TODO: Implémentation avec base de données pour stocker l'historique des rapports
            # Pour l'instant, retournons une liste simulée
            recent_reports = [
                {
                    'id': 1,
                    'name': 'Rapport de Valorisation Stock',
