...
class AlerteController:
    ...
    def get_current_alerts(self, session=None) -> List[Dict[str, Any]]:
        """
        Récupérer toutes les alertes actuelles (stock faible + surstock)
        
        Args:
            session: Session SQLAlchemy (ignorée pour compatibilité)
            
        Returns:
            Liste combinée de toutes les alertes actuelles
        """
        try:
            # Récupérer les alertes de stock faible
            ...
            # Récupérer les alertes de surstock
            overstock_alerts = self.get_overstock_alerts()
            ...
            # Ajouter les alertes de surstock
            ...
            all_alerts.sort(key=lambda x: priority_order.get(x.get('alert_level', 'low'), 4))
            
            return all_alerts
            
        except Exception as e:
            print(f"Erreur lors de la récupération des alertes actuelles: {e}")
            return []