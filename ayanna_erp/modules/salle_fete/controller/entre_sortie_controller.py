    ...
    def create_expense(self, expense_data):
        ...
        try:
            ...
            if not config:
                ...
                # -- Résoudre les comptes AVANT de créer le journal --
                # Le compte sélectionné dans le combo 'compte_id' : compte de charge (débit).
                # Le compte financier 'compte_financier_id' : contrepartie (crédit).
                # Fallback : compte_caisse_id de ComptaConfig, puis compte_selected.
                ...
                # Déterminer le compte crédit (financier)
                credit_account = None
                if compte_financier:
                ...
                # Type de journal automatique selon le numéro du compte financier (SYSCOHADA)
                # 57x (trésorerie/banque) → 'banque', sinon → 'caisse'
                type_journal = self._resolve_journal_type(getattr(credit_account, 'numero', None))

                # Créer la ligne de journal comptable
                libelle = f"{expense.description}"
                ...
                if current_balance is not None and current_balance < montant_a_payer:
                    ...
                    err = (
                        ...
                        f"Solde actuel: {current_balance:.2f} F — Montant demandé: {montant_a_payer:.2f} F.\n"
