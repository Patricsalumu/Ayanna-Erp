...
class EnterpriseFormWidget(QDialog):
    ...
    def create_general_info_group(self):
        ...
        self.taux_edit.setPlaceholderText("Ex: 2200 (1 USD = 2200 FC). Laisser vide pour désactiver.")
        layout.addRow("Taux de change:", self.taux_edit)
    ...
    def populate_fields(self):
        ...
        if self.mode == "edit" and self.enterprise_data:
            ...
            # Taux de change
            taux = self.enterprise_data.get('taux_de_change')
            self.taux_edit.setText(str(taux) if taux is not None else '')
    ...
    def collect_data(self):
        ...
        taux_str = self.taux_edit.text().strip()
        taux_val = None
        if taux_str:
            try:
                taux_val = float(taux_str.replace(',', '.'))
            except ValueError:
                taux_val = None
        data = {
