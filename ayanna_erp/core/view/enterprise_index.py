...
class EnterpriseIndexView(QWidget):
    ...
    def create_logo_section(self, data):
        ...
        if logo_blob:
            # Convertir le BLOB en pixmap
            try:
                from ayanna_erp.core.utils.image_utils import ImageUtils
                pixmap = ImageUtils.blob_to_pixmap(logo_blob)
                ...
                if pixmap:
                    ...
                    info = ImageUtils.get_image_info(logo_blob)
                    info_text = ""
                    if info:
                        info_text = f"Format: {info.get('format', 'Inconnu')} | "
                        info_text += f"Taille: {info.get('width', 0)}x{info.get('height', 0)} | "
                        info_text += f"Données: {info.get('data_size', 0)} bytes"
                else:
                    logo_label.setText("❌ Erreur de chargement du logo")
                    info_text = "Données corrompues"
                    
            except Exception as e:
                logo_label.setText(f"❌ Erreur: {str(e)}")
                info_text = "Erreur de traitement"
        else:
            logo_label.setText("📷 Aucun logo configuré\n\nUtilisez le bouton 'Modifier' pour ajouter un logo")
            info_text = "Aucune donnée de logo"
        
        layout.addWidget(logo_label)
        
        # Information sur le logo
        info_label = QLabel(info_text if logo_blob else "Aucun logo")
    ...
    def create_enterprise(self):
        ...
        dialog = EnterpriseFormWidget(self, mode="create")
