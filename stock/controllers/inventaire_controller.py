...
class InventaireController:
    ...
    def create_inventory_session(self, session: Session, inventory_data: Dict[str, Any]) -> Optional[StockInventaire]:
        ...
        try:
            ...
            # Récupérer la liste initiale des produits selon le type (complet/partiel/par catégorie)
            products = []
            all_products = self.get_products_for_inventory(warehouse_id)

            inv_type_raw = str(inventory_data.get('inventory_type', '') or '').lower()
            inv_type = inv_type_raw.replace('é', 'e').replace('è', 'e').replace('ê', 'e').replace('à', 'a')
            ...
            elif 'categorie' in inv_type:
                selected_categories = set(inventory_data.get('category_ids') or [])
                for p in all_products:
